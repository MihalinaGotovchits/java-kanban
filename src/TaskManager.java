...
public class TaskManager {
    ...
    public Task foundTaskInSimpleTasksById (int taskId){//получение задачи по индентификатору из списка простых задач
        ...
        if (taskId == 0 || simpleTasks.isEmpty()){
            ...
            if (simpleTasks.containsKey(taskId)){
                task = simpleTasks.get(taskId);//вывести задачу
            }
        }
        return task;
    }

    public Epic foundTaskInEpicTasksById (int id){
    ...
    public Subtask foundTaskInSubTasksById (int id){
